...
#else
	memcpy((uint8_t *)&main_par, (uint8_t *)&data[WATER7_USERPARAMS_OFFSET * 2], sizeof(main_par_t));
	return 0;
#endif
}
...
#else
	memcpy((uint8_t *)&data[WATER7_USERPARAMS_OFFSET * 2], (uint8_t *)&main_par, sizeof(main_par_t));
	//WVT_EEPROM_LoadAll(data, EEPROM_WATER7_PARAMS);
	return 0;
#endif
}

int32_t water7save_data(int32_t *data)
{
	/// \todo check this
...
#else
			memcpy((uint8_t *)cache, (uint8_t *)(page * FLASH_PAGE_SIZE + FLASH_BASE), FLASH_PAGE_SIZE);
#endif
		}
...
#else
			memcpy((uint8_t *)&cache[addr % FLASH_PAGE_SIZE], (uint8_t *)data, len_first);
#endif
...
#else
			memcpy((uint8_t *)cache, (uint8_t *)(page_next * FLASH_PAGE_SIZE + FLASH_BASE), FLASH_PAGE_SIZE);
			memcpy((uint8_t *)cache, (uint8_t *)&data[len_first], len_second);
#endif
		}
		else
		{
...
#else
			memcpy((uint8_t *)&cache[addr % FLASH_PAGE_SIZE], (uint8_t *)data, len);
#endif
		}
		page_prev = page_next;
	}
